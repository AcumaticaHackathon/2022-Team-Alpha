...
CREATE OR ALTER TRIGGER Insert_GIOn
   ON GIOn
   AFTER INSERT
AS 
BEGIN
	...
	INSERT INTO @loopKeys (CompanyID, DesignID, RelationNbr, LineNbr)
	SELECT CompanyID, DesignID, RelationNbr, LineNbr FROM inserted
	...
	BEGIN
		...
		SELECT TOP 1 
			@companyID = CompanyID,
			@designID = DesignID,
			@relationNbr = RelationNbr,
			@lineNbr = LineNbr
		FROM @loopKeys

		-- Build JSON key of current row
		SELECT @jsonKey = (
			SELECT CompanyID, DesignID, RelationNbr, LineNbr
			FROM @loopKeys
			WHERE CompanyID = @companyID
				AND DesignID = @designID
				AND RelationNbr = @relationNbr
				AND LineNbr = @lineNbr
		...
		WHERE CompanyID = @companyID
			...
			AND CAST(JSON_VALUE(KeyValues, '$.RelationNbr') AS int) = @relationNbr
