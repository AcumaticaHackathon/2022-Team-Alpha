...
CREATE OR ALTER TRIGGER Update_RolesInMember
   ON RolesInMember
   AFTER UPDATE
AS 
BEGIN
	...
	INSERT INTO @loopKeys (CompanyID, RoleName, ApplicationName, ScreenID, CacheType, MemberName)
	SELECT CompanyID, RoleName, ApplicationName, ScreenID, CacheType, MemberName FROM inserted
	...
	BEGIN
		...
		SELECT @jsonKey = (
			SELECT CompanyID, RoleName, ApplicationName, ScreenID, CacheType, MemberName
			FROM @loopKeys
		...
		WHERE CompanyID = @companyID
			...
			AND CAST(JSON_VALUE(KeyValues, '$.MemberName') AS varchar(255)) = @memberName
