...
CREATE OR ALTER TRIGGER Insert_BPEventSubscriber
   ON BPEventSubscriber
   AFTER INSERT
AS 
BEGIN
	...
	INSERT INTO @loopKeys (CompanyID, EventID, HandlerID)
	SELECT CompanyID, EventID, HandlerID FROM inserted
	...
	BEGIN
		...
		SELECT @jsonKey = (
			SELECT CompanyID, EventID, HandlerID
			FROM @loopKeys
		...
		WHERE CompanyID = @companyID
			...
			AND CAST(JSON_VALUE(KeyValues, '$.HandlerID') AS uniqueidentifier) = @handlerID
