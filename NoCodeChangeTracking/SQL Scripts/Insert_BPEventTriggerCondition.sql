...
CREATE OR ALTER TRIGGER Insert_BPEventTriggerCondition
   ON BPEventTriggerCondition
   AFTER INSERT
AS 
BEGIN
	...
	WHERE [Name] = 'BPEventTriggerCondition'
	...
	INSERT INTO @loopKeys (CompanyID, EventID, OrderNbr)
	SELECT CompanyID, EventID, OrderNbr FROM inserted
	...
	BEGIN
		...
		SELECT @jsonKey = (
			SELECT CompanyID, EventID, OrderNbr
			FROM @loopKeys
			WHERE CompanyID = @companyID
				AND EventID = @eventID
				AND OrderNbr = @orderNbr
		...
		WHERE CompanyID = @companyID
			...
			AND CAST(JSON_VALUE(KeyValues, '$.OrderNbr') AS smallint) = @orderNbr
