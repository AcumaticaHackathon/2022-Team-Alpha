...
CREATE OR ALTER TRIGGER Insert_WebHook
   ON WebHook
   AFTER INSERT
AS 
BEGIN
	...
	INSERT INTO @loopKeys (CompanyID, WebHookID)
	SELECT CompanyID, WebHookID FROM inserted
	...
	BEGIN
		...
		SELECT @jsonKey = (
			SELECT CompanyID, WebHookID
			FROM @loopKeys
			WHERE CompanyID = @companyID
				AND WebHookID = @webHookID
		...
		WHERE CompanyID = @companyID
			...
			AND CAST(JSON_VALUE(KeyValues, '$.WebHookID') AS uniqueidentifier) = @webHookID
