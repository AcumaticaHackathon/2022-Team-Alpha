...
CREATE OR ALTER TRIGGER Insert_BPEventSchedule
   ON BPEventSchedule
   AFTER INSERT
AS 
BEGIN
	...
	INSERT INTO @loopKeys (CompanyID, EventID, ScheduleID)
	SELECT CompanyID, EventID, ScheduleID FROM inserted
	...
	BEGIN
		...
		SELECT @jsonKey = (
			SELECT CompanyID, EventID, ScheduleID
			FROM @loopKeys
		...
		WHERE CompanyID = @companyID
			...
			AND CAST(JSON_VALUE(KeyValues, '$.ScheduleID') AS int) = @scheduleID
