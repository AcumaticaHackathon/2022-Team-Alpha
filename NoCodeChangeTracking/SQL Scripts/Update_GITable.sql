...
CREATE OR ALTER TRIGGER Update_GITable
   ON GITable
   AFTER UPDATE
AS 
BEGIN
	...
	INSERT INTO @loopKeys (CompanyID, DesignID, Alias)
	SELECT CompanyID, DesignID, Alias FROM inserted
	...
	BEGIN
		...
		SELECT @jsonKey = (
			SELECT CompanyID, DesignID, Alias
			FROM @loopKeys
		...
		WHERE CompanyID = @companyID
			...
			AND CAST(JSON_VALUE(KeyValues, '$.Alias') AS nvarchar(255)) = @alias
