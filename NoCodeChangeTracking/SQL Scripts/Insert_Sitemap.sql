...
CREATE OR ALTER TRIGGER Insert_Sitemap
   ON Sitemap
   AFTER INSERT
AS 
BEGIN
	...
	INSERT INTO @loopKeys (CompanyID, NodeID)
	SELECT CompanyID, NodeID FROM inserted
	...
	BEGIN
		...
		WHERE CompanyID = @companyID
			...
			AND CAST(JSON_VALUE(KeyValues, '$.NodeID') AS uniqueidentifier) = @nodeID
