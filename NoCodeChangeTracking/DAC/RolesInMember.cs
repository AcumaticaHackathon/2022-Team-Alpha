...
{
    /// <summary>
    /// RolesInMember table adapter for tracking changes
    /// </summary>
    [Serializable]
    [PXCacheName("RolesInMember")]
    public class RolesInMember : IBqlTable, ITrackedObject
    {
        #region ScreenID
        ...
        public bool IsKeyMatch(dynamic key, object tableRow)
        {
            ...
            var roleInMember = (RolesInMember)tableRow;
            ...
            if (key != null)
            {
                ...
                memberName = Convert.ToString(key.MemberName);
            }

            return ((!string.IsNullOrEmpty(roleName)) && roleInMember.Rolename == roleName
                && (!string.IsNullOrEmpty(applicationName)) && roleInMember.ApplicationName == applicationName
                && (!string.IsNullOrEmpty(screenID)) && roleInMember.ScreenID == screenID
                && (!string.IsNullOrEmpty(cacheType)) && roleInMember.Cachetype == cacheType
                && (!string.IsNullOrEmpty(memberName)) && roleInMember.Membername == memberName);
        }
